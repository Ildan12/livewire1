    ...
    <section class="text-gray-600 body-font">
        <div class="container px-5 py-8 mx-auto">
            ...
            <div class="flex flex-col text-center w-full mb-8">
                ...
                <p class="text-gray-500 mt-2">Управление категориями товаров</p>
            </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 p-4 bg-white rounded-lg shadow-sm border">
                ...
                <div class="relative flex-1 max-w-md">
                    ...
                    <input wire:model.live.debounce.300ms='search' 
                           ...
                           class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 placeholder-gray-400">
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($categories as $category)
                            <tr wire:key='{{ $category->id }}'
                                class="grid grid-cols-[80px_1fr_120px] hover:bg-blue-50 transition-colors duration-200">
                                <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ $category->id }}</td>
                                <td class="px-4 py-4 text-sm text-gray-900">{{ $category->name }}</td>
                                <td class="px-4 py-4 text-sm">
                                    <button wire:click='deleteCategory({{ $category->id }})'
                                            wire:confirm="Вы уверены, что хотите удалить категорию '{{ $category->name }}'?"
                                            class="text-red-600 hover:text-red-800 transition-colors flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-red-50"
            ...
            @if($categories->isEmpty())
                <div class="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Категории не найдены</h3>
